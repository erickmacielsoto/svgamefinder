...
# Para detectar modo oscuro en Windows
try:
    import winreg
except ImportError:
    winreg = None
...
def detectar_modo_oscuro_sistema():
    # Solo para Windows por ahora, macOS y Linux requieren otras libs
...
def actualizar_encabezados():
    # Title IDs
    tree_title_ids.heading("title_id", text=traducir("col_title_id"))
    tree_title_ids.heading("name", text=traducir("col_name"))
    tree_title_ids.heading("system", text=traducir("col_system"))
    # Releases
    tree_releases.heading("name", text=traducir("col_name"))
    tree_releases.heading("regions", text=traducir("col_regions"))
    tree_releases.heading("system", text=traducir("col_system"))
...
def actualizar_textos():
    switch.configure(text=traducir("modo_oscuro"))
    label_entrada.configure(text=traducir("ingresa_texto"))
    boton_buscar.configure(text=traducir("buscar"))
    label_title.configure(text=traducir("title_ids"))
    label_releases.configure(text=traducir("releases"))
    idioma_menu_label.configure(text=f"🌐 {traducir('idioma')}:")
    menu_title.entryconfig(0, label=traducir("copiar_celda"))
    menu_title.entryconfig(1, label=traducir("copiar_fila"))
    menu_release.entryconfig(0, label=traducir("copiar_celda"))
    menu_release.entryconfig(1, label=traducir("copiar_fila"))
...
# Cargar configuración o usar sistema
